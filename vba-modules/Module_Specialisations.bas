...
Public Function ObtenirGuidesAutorises(typeVisite As String) As Collection
    ...
    For i = 2 To derLigne ' Ligne 2 = premier guide (apres en-tetes)
        nomGuide = Trim(wsGuides.Cells(i, 1).Value) & " " & Trim(wsGuides.Cells(i, 2).Value) ' Prenom + Nom

        If nomGuide <> " " Then ' Verifier qu'il y a du contenu
