...
Public Sub GenererContratFinMois()
    ...
    For i = 2 To wsCalculs.Cells(wsCalculs.Rows.Count, 1).End(xlUp).Row
        If wsCalculs.Cells(i, 1).Value = guideID Then
            ...
            ' Verifier si colonnes defraiements existent (colonne N = 14)
            On Error Resume Next
    ...
    ' Ajouter ligne defraiements si montant > 0
    If defraiements > 0 Then
        ligne = ligne + 1
        wsContrat.Cells(ligne, 1).Value = "Defraiements :"
        wsContrat.Cells(ligne, 1).Font.Bold = True
    ...
    If defraiements > 0 Then
        detailCalcul = detailCalcul & " + Defraiements " & Format(defraiements, "#,##0.00") & " € = " & Format(montantTotalAvecFrais, "#,##0.00") & " €"
    End If
